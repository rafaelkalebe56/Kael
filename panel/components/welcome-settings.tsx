...
export function WelcomeSettings({ guildId }: { guildId: string }) {
  ...
  if (state === 'denied') return <div className="panel-empty guild-access-state"><span className="panel-empty-icon"><KaelShield /></span><p className="panel-state-label">ACESSO PROTEGIDO</p><h2>Você não pode configurar este servidor.</h2><p>O Kael confirmou suas permissões novamente e bloqueou esta página.</p><button className="panel-secondary-link" type="button" onClick={() => location.assign('/servidores')}><KaelArrowLeft /> Voltar aos servidores</button></div>;
